...
-(void)httpNumber
{
    ...
    NSString * strUrl = [NSString stringWithFormat:@"http://139.210.99.29:8090/query.aspx?id=3"];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (alertView.tag == 101) {
        if (buttonIndex==0) {
            //app 下载网址
            //https://itunes.apple.com/us/app/china-inheritance-health/id794167439?ls=1&mt=8
            
            NSString *webLink = @"https://itunes.apple.com/us/app/china-inheritance-health/id794167439?ls=1&mt=8";
...
-(void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    if (![stringNum isEqualToString:versionNum]) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Prompt"
                                                       message:@"Go to update to the latest version is detected!"
                                                      delegate:self
