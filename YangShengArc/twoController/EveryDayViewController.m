...
-(void)updateMeters{
    ...
    if (y >= second) {
        ...
        if ([fd isEqualToString:@"100"]) {
            eView.nameLabel.text = @"Title:初窥门径";
            ...
            eView.nameLabel.text = @"Title:略有小成";
            ...
            eView.nameLabel.text = @"Title:驾轻就熟";
            ...
            eView.nameLabel.text = @"Title:炉火纯青";
            ...
            eView.nameLabel.text = @"Title:出类拔萃";
            ...
            eView.nameLabel.text = @"Title:神乎其技";
            ...
            eView.nameLabel.text = @"Title:傲视群雄";
            ...
            eView.nameLabel.text = @"Title:登峰造极";
            [defaults setObject:@"登峰造极" forKey:@"name"];
            [self.timer invalidate];
            self.timer = nil;

        }if ([fd isEqualToString:@"900"]) {
            eView.nameLabel.text = @"Title:所向披靡";
            ...
            eView.nameLabel.text = @"Title:一代宗师";
            ...
            eView.nameLabel.text = @"Title:神功盖世";
...
//    }
    ...
    if (sTime != nil) {
        ...
        self.strTime = @"Set time!";
    }
    
    if (sName != nil) {
        eView.nameLabel.text = [NSString stringWithFormat:@"Title:%@",sName];
    }else{
        eView.nameLabel.text = @"Title:初学乍练";
    }
    if (sFen != nil) {
        eView.fenLabel.text = [NSString stringWithFormat:@"Experience:%@",sFen];
    }else{
    
        eView.fenLabel.text = @"Experience:无";
    }
    
    
    
    eView.lgTimeLab.text = [NSString stringWithFormat:@"Alarm time:%@",self.strTime];
  ...
  //  eView.jieShLab.text =@"拉萨的控件佛网噢诶人哦我很烦小才俊恐惧啊欧文哦看见我合法可使肌肤看见啊束带结发哈史蒂夫是";
    ...
    NSString * names = @"Qiangshen waist power";
    NSString * jieNais = @"Qiangshen waist Power: Essence collection Shaolin Ba Duan Jin, Wu Qin Xi, tai chi and other traditional regimen of exercises, solid to complement deficiency, kidney essence deficiency who have significant effect!";
...
-(void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    if (request.tag == 1111) {
        ...
        UIAlertView* alertView=[[UIAlertView alloc]initWithTitle:@"Success"
														 message:@"Video download is complete"
														delegate:nil
...
-(void)requestFailed:(ASIHTTPRequest *)request
{
    ...
    lThreebool = NO;
        ...
        UIAlertView* alertView=[[UIAlertView alloc]initWithTitle:@"error"
														 message:@"Network Anomaly! Please download later!"
														delegate:nil
											   cancelButtonTitle:@"ok"
											   otherButtonTitles:nil];
		[alertView show];
...
-(void)xiazaiBtn:(MyButton * )btn
{
    ...
    if (self.lieArrayJson.count != 0 && rOnebool != YES) {
...
//        }
        ...
        UIAlertView* alertView=[[UIAlertView alloc]initWithTitle:@"Downloading"
														 message:@"Please wait for the download is complete!"
														delegate:nil
											   cancelButtonTitle:@"ok"
											   otherButtonTitles:nil];
		[alertView show];
